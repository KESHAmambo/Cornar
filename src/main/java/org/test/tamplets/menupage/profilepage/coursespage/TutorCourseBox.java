package org.test.tamplets.menupage.profilepage.coursespage;

import com.vaadin.annotations.AutoGenerated;
import com.vaadin.annotations.DesignRoot;
import com.vaadin.ui.declarative.Design;
import com.vaadin.ui.VerticalLayout;
import com.vaadin.ui.Label;
import com.vaadin.ui.Button;

/**
 * !! DO NOT EDIT THIS FILE !!
 * <p>
 * This class is generated by Vaadin Designer and will be overwritten.
 * <p>
 * Please make a subclass with logic and additional interfaces as needed,
 * e.g class LoginView extends LoginDesign implements View { }
 */
@DesignRoot
@AutoGenerated
@SuppressWarnings("serial")
public class TutorCourseBox extends VerticalLayout {
    protected VerticalLayout mainLayout;
    protected Label courseNameLabel;
    protected Button addLessonButton;
    protected Button showLessonsButton;
    protected Label tutorNameLabel;
    protected Label descriptionLabel;

    public TutorCourseBox() {
        Design.read(this);
    }

    public Button getSeeLessonsButton() {
        return showLessonsButton;
    }

    public Button getAddLessonButton() {
        return addLessonButton;
    }
}
